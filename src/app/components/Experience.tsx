...
const Experience = () => {
	...
	useEffect(() => {
		...
		datos.forEach((elem) => {
			animateText(
				elem as HTMLElement,
				(elem as HTMLElement).textContent || ""
			);
		});
	}, [expData]);
	...
	return (
		<div id="experience">
			...
			<section
				id="experience"
				className="min-h-screen lg:bg-[url('/fondos/exp-fondo2.webp')] bg-fixed bg-cover bg-no-repeat px-5 py-10"
			>
				<div className="flex flex-col lg:flex-row gap-5 justify-center items-center w-full min-h-screen">
					<div className="w-full lg:w-1/4 flex flex-col gap-5 p-5 justify-center items-center">
						{companias.map((elem, index) => (
                            ...
                            ${
								selectedCompany === elem
									? "bg-gray-300 scale-115 hover:scale-115"
									: "bg-[rgb(240,240,240)]"
							}
                            `}
									id={elem}
								>
									<div className="h-full w-full flex justify-center items-center">
										<p className="font-[carbon] text-2xl md:text-3xl lg:text-4xl">
											{elem === "-" ? "Skills" : elem}
										</p>
									</div>
								</button>
					...
					<div className="w-full lg:w-3/4 flex flex-col p-5 justify-start items-start flex-grow">
						<FadeInSection>
							<h3
								className="animate-data text-2xl md:text-3xl lg:text-5xl mb-2 font-[carbon]"
								id="position"
							>
								{expData?.position}
							</h3>
						...
						<FadeInSection>
							<p
								className="animate-data text-lg md:text-xl lg:text-2xl font-light mb-5 font-[FiraCode]"
								id="company"
							>
								{expData?.company === "Tata"
									? "Tata Consultancy Services"
									: expData.company}
							</p>
						</FadeInSection>
						<FadeInSection viewAmount={0.4}>
							<ul className="ml-4 list-disc">
								{expData?.bulletpoints.map((point, index) => (
									<li
										className="animate-data text-sm md:text-lg lg:text-xl tracking-tight font-light mb-1 font-[FiraCode]"
										key={index}
										id={`bullet-${index}`}
									>
										{point}
